...
try:
    ...
    # 404 에러 방지를 위해 가장 안정적인 1.5-flash 모델로 설정
    # (배포 환경에 따라 gemini-1.5-flash 또는 gemini-1.5-flash-latest 사용)
    model = genai.GenerativeModel("gemini-1.5-flash")
except Exception as e:
    st.error(f"⚠️ 설정 오류: {str(e)}. Streamlit Cloud의 Settings > Secrets를 확인해주세요.")
    st.stop()

# 3. JSON 추출 강화 함수 (Extra Data 및 파싱 에러 완벽 방어)
...
urls_input = st.text_area("Target URLs (한 줄에 하나씩 입력)", height=100, placeholder="[https://classys.co.kr](https://classys.co.kr)")
        ...
        try:
            ...
            for i, tone in enumerate(tones):
                with cols[i]:
                    ...
                    st.link_button(f"🔍 '{q}' 테스트", f"[https://www.genspark.ai/search?query=](https://www.genspark.ai/search?query=){encoded_q}", use_container_width=True)
